...
student_df['Full Name'] = student_df['First Name'] + \
    " " + student_df['Last Name']
...
student_df['Books Reading'] = student_df['E-Library Book ID'].apply(
    lambda x: ', '.join([book_id_to_name.get(book_id.strip(), 'Unknown')
                        for book_id in str(x).split(',')])
)
...
student_df['Encoded Books Reading'] = label_encoder.fit_transform(
    student_df['Books Reading'])
...
# Streamlit App
st.set_page_config(
    page_title="Student Group Clustering Dashboard", layout="wide")
...
selected_student_name = st.sidebar.selectbox(
    "Select a student:", student_df['Full Name'].values)
selected_student = student_df[student_df['Full Name']
                              == selected_student_name].iloc[0]
...
with col1:
    st.markdown(
        f"**🎓 Degree Program:** {original_degree[selected_student.name]}")
...
with col3:
    st.markdown(
        f"**📅 Enrollment Date:** {selected_student['Enrollment Date']}")
    st.markdown(
        f"**🔖 Status:** {'Active' if selected_student['Status'] == 1 else 'Inactive'}")

# Cluster Overview
st.subheader("📊 Cluster Overview")
for cluster_id, group in student_df.groupby('Cluster'):
    with st.expander(f"{group_names[cluster_id]} ({len(group)} Students)"):
        # Revert encoding back to original names for Degree and Faculty
        group['Degree'] = group['Degree'].apply(
            lambda x: label_encoder.inverse_transform([x])[0])
        group['Faculty'] = group['Faculty'].apply(
            lambda x: label_encoder.inverse_transform([x])[0])

        # Display the cluster data with the original names
        st.dataframe(group[['Full Name', 'Faculty', 'Degree',
                     'Books Reading']].reset_index(drop=True))
